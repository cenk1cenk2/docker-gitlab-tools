...
ARG VERSION=09b7cfecd91c791f358ee1fe1e71f4c755747a26
...
RUN \
  ...
  apt-get update && \
  apt-get install -y python3-virtualenv virtualenv build-essential python3-dev libpq-dev git postgresql-server-dev-all python3-pip libffi-dev nginx uwsgi uwsgi-plugin-python3 nodejs && \
...
ENTRYPOINT [ "vizier", "--config-file", "/etc/vizier.yml" ]
